...
def index():
    ...
    for stock in stocks:
        ...
        grand_total += round(stock["value"], 3)
...
def buy():
    ...
    if request.method == "POST":
        ...
        if not symbol:
            ...
            return apology("Provide positive integer for shares")
        quote = lookup(symbol)
        if quote is None:
        ...
        flash(f"You have bought {shares} of {symbol} for {usd(total_cost)}!")
        return redirect("/")
        ...
        for stock in stocks:
            if stock["symbol"] == symbol:
                if stock["total_shares"] < shares:
                    ...
                    flash(
                        f"You have sold {shares} shares of {symbol} for {usd(total_sale)}!"
                    )
                    return redirect("/")
